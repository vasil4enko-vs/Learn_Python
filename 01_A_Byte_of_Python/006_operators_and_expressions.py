...
операторов и операндов, чтобы в явном виде указать порядок вычисления
выражений. Заодно это облегчит чтение программы. Более подробно см. в разделе
Изменение порядка вычисления ниже.
...
*, /, //, % 	              |  Умножение, деление, целочисленное деление и
                              | остаток от деления
...
""" Операторы обычно обрабатываются слева направо. Это означает, что операторы
с равным приоритетом будут обработаны по порядку от левого до правого.
Например, 2 + 3 + 4 обрабатывается как (2 + 3) + 4.
...
""" [1] "True" - англ. "Верно (Правда)"; "False" - англ. "Ошибочно (Ложь)".
    (прим. перев.) [2] "tuple" - англ. "кортеж" (прим. перев.)
